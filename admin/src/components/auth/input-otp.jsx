...
export default function OtpInput({
  ...
  return (
    <Card className="gap-4 shadow-none border-0">
      <CardHeader>
        <CardTitle>
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 p-">
          ...
          >
            {isLoading ? (
              <>
                <Loader className="animate-spin size-6" /> Verifying OTP....
              </>
            ) : (
              "Verify OTP"
            )}
        ...
        <Button variant="outline" onClick={handleRequestOtp} className="mt-2">
          {sendingOtp ? (
            <>
