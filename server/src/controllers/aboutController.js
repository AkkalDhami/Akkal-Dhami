  ...
  try {
    ...
    if (type === "socials") {
      ...
      return res.status(400).json({
        success: false,
        message: "Invalid type. Must be 'socials' or 'ctaButtons'.",
      });
    }
    ...
    res.status(500).json({
      success: false,
      message: "Error adding social/cta button",
      error,
    });
  }
};
...
export const removeSocialOrCta = async (req, res) => {
  try {
    ...
    if (type === "socials") {
      about.socials = about.socials.filter(
        (item) => item._id.toString() !== itemId
      );
    } else if (type === "ctaButtons") {
      about.ctaButtons = about.ctaButtons.filter(
        (item) => item._id.toString() !== itemId
      );
    } else {
      return res.status(400).json({
    ...
    res.status(500).json({
      success: false,
      message: "Error removing social/cta button",
      error,
    });
  }
};
...
export const addMyContactDetails = async (req, res) => {
  try {
    ...
    const isAboutExist = await About.countDocuments({});
    if (isAboutExist > 0)
      return res.status(400).json({
        success: false,
        message: "Basic info already exist",
      });

    const myContact = new About({
      name,
      email,
      contact,
      location,
    ...
    res.status(201).json({
      success: true,
      message: "Basic Info Added Sucessfully!",
    });
  } catch (error) {
    res.status(500).json({
      success: false,
...
export const updateMyContactDetails = async (req, res) => {
  try {
    ...
    const { name, email, contact, location, isAvailable, roles, shortIntro } =
      req.body;
    await About.findByIdAndUpdate(
      id,
      { name, email, contact, location, isAvailable, roles, shortIntro },
      { new: true, runValidators: true }
    );
    res.status(201).json({
      success: true,
      message: "Basic Info Updated Sucessfully!",
    });
  } catch (error) {
    res.status(500).json({
      success: false,
